...
#        action = self.get_formview_action(self.id)
...
class InheritViewChange(models.Model):
    ...
    inherit_view_type = fields.Selection([('field', 'Field'), ('xpath', 'XPath')], 'Selection Type', default='field',
                                         required=True)
    inherit_view_target = fields.Char('Inherit Target/Expr', required=True , default='//form/sheet[1]/notebook[1]/page[2] OR //field[@name=\'FIELDNAME\']')
    ...
    inherit_view_field = fields.Char('Field', default='<field name="score_line_ids" widget="many2many_kanban" attrs="{\'invisible\':[(\'type\',\'!=\',\'free_text\'), (\'type\',\'!=\',\'textbox\')]}" /> OR  <button class="oe_stat_button" type="action" icon="fa-tasks" name="%(action_todo_task_button)d" context="{\'default_user_id\': user_id}" help="All to-dos for this user" > <field string="To-Dos" name="user_todo_count" widget="statinfo"/> </button>')
    #inherit_view_field = fields.Char('Field')
    inherit_view_page = fields.Boolean('Page' , default=False)
    inherit_view_pagename = fields.Char('Page Name' , placeholder="e.g. pagename")
    inherit_view_pagestring = fields.Char('Page String' , placeholder="e.g. Page String")
    inherit_view_group = fields.Boolean('Group', default=False )
