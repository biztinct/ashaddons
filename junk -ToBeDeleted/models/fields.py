...
#Ashrevert    ttype = fields.Selection(selection='get_field_types', string='Field Type', required=True )
    ttype = fields.Selection([('binary','binary'),('boolean','boolean'),('char','char'),('date','date'),('datetime','datetime'),('float','float'),('html','html'),('integer','integer'),('monetary','monetary'),('reference','reference'),('selection','selection'),('serialized','serialized'),('text','text'),('many2one', 'many2one'), ('one2many', 'one2many'), ('many2many', 'many2many')], string='Field Type', required=True )
...
#Ash    default_method_name = fields.Char('Default Method Name')
    default_method_name = fields.Char('Default')
...
#A            if type(v) == types.TypeType and \
# A           issubclass(v, fields_old._column) and v != fields_old._column and \
#A            not v._deprecated and \
            # not issubclass(v, fields_old.function)])
#A            not issubclass(v, fields_old.function) and \
#A            (not context.get('from_diagram', False) or (
#A                context.get('from_diagram', False) and (k in ['one2many', 'many2one', 'many2many'])))
        ])
...
# Ash        self.default_method_name = "_default_{field}".format(field=self.name)
        self.default_method_name = "eg True OR 5 OR 'Text Value'"
